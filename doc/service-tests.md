...
2. We use the `get()` method to request a badge. There are several points to consider here:
   ...
   - All badges on shields can be requested in a number of formats. As well as calling https://shields.eplus.dev/wercker/build/wercker/go-wercker-api.svg to generate ![](https://shields.eplus.dev/wercker/build/wercker/go-wercker-api.svg) we can also call https://shields.eplus.dev/wercker/build/wercker/go-wercker-api.json to request the same content as JSON. When writing service tests, we request the badge in JSON format so it is easier to make assertions about the content.
